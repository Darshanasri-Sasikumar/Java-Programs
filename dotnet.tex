...
| ------------------------------ | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------ |
| **Purpose**                    | Used to pass a variable **by reference** to a method, so the method can read and modify the value.     | Used to pass a variable **by reference**, but mainly to **return data** from a method.                                         |
| **Initialization Requirement** | The variable **must be initialized** before it is passed to the method.                                | The variable **does not need to be initialized** before being passed, but the method **must assign a value** before returning. |
| **Use Case**                   | When you want the method to **read and optionally modify** an existing value.                          | When the method needs to **output a value** back to the caller.                                                                |
| **Value Assignment in Method** | The method **can read and write** to the variable.                                                     | The method **must assign a value** before exiting.                                                                             |
| **Compiler Behavior**          | Compiler checks that the variable is initialized before being passed.                                  | Compiler checks that the variable is assigned a value inside the method before returning.                                      |
| **Example**                    | `csharp<br>void Increment(ref int x) { x = x + 1; }<br>int a = 5;<br>Increment(ref a); // a becomes 6` | `csharp<br>void GetValues(out int x, out int y) { x = 10; y = 20; }<br>int a, b;<br>GetValues(out a, out b); // a=10, b=20`    |
| **Number of Assignments**      | Can be assigned **zero or more times** inside the method.                                              | Must be assigned **at least once** before the method ends.                                                                     |
...
Delegates :
       ...
       it acts as an refference to a method
...
Eager Loading:
                ->eager Loading is used to load the related data along with the maindata
                ->Data is fetched in Advanced
                ->reuces multiple database calls
                ->Improves performance when data is needed
Lazy Loading:
                ->lazy loading is used to load the data when ever it is required
                ->data fetched on demand 
                ->time consuming
                ->permance might be bit slow since after call it fetches the data
                ->More database calls
...
| More control              | Easy and readable         |
| Best when index is needed | Best for simple iteration |


Class

                A Class is a reference type used to define objects that contain data and behavior.
                Key Points:
                Reference type
                Stored in heap memory
                Supports inheritance
                Can contain methods, properties, constructors, destructors
...
A Struct is a value type used to store small data objects.
                ...
                Used for small and simple data
...
| Memory freed automatically | Memory freed by GC     |

Iemurable and Iquerable
                Both are used to retrive the data from the database
IEmurable:
        ->perform filtering after data is loaded
        ->it is used to retrive small level of database
        Fetches all data first

Iquerable 
        ->IQuerable is to load the data at the retraival in databse level itself
        But when we need database-level filtering and better performance, especially with large datasets, we should use IQueryable
        Fetches required data only

Global exceptions handling ->middeware ahh pottruva

Garbage Collector :
                ->Garbage collector is a memory managemnt features
                ->it is used to remove objects that is not used in the application for longer period of time
                ->It help to free the memory in the heap area
                ->it helps in increase the performance
                ->it helps prevent the memory leak

Custom Exception:
                ->User define Exception is know as custom exceptions
                ->created by Built in Exception class
                ->we used it when the default exception does not meets the requiremnt we will go custom exception

         Why we use Custom Exceptions

                To represent business rule violations
                To provide clear and meaningful error messages
                To improve readability, debugging, and maintainability

Attributes:
        Attributes are meta datas of tags
        It is used to add extra information
        ->it doesnot change the logic
...
JWT (JSON Web Token)
JWT is a JSON Web Token used for authentication and authorization.
It acts as a gatekeeper after login, not during credential validation.

When a user logs in, the server verifies the credentials (email & password).
If they are correct, the server generates a JWT token and sends it to the client.

The token is valid for a limited time (minutes or days).
For every request, the client sends this token to access authorized data.

If the token is valid, access is allowed.
If the token is missing, expired, or invalid, the server returns an error
(commonly 401 Unauthorized or 403 Forbidden, not 422).


View data and view back set data
Agylisys

Routing:
        Routing directs an HTTP request to the correct controller so that the appropriate response is returned.
        Conventional Routing:Routing will be done centralized

        Attribute Routing:
        Attribute routing defines routes directly on controllers or action methods using attributes.
        This gives more control and improves readability.

What is Model Binding?

Model Binding automatically takes data from an HTTP request and puts it into a model or object.

ViewData:ViewData is used to pass data from Controller to View for one request only,Stored as key–value pair,Needs type casting

ViewBag:ViewBag is a dynamic object used to pass data from Controller to View for one request.Useful for redirect scenarios,No type casting needed,Easier than ViewData,Works only for the current request

TempData:TempData is used to pass data between two requests,Useful for redirect scenarios,Stored temporarily,Data is removed after it is read


What is an Identity Column?

An Identity column automatically generates a unique number for each new row in a table..
->To generate auto-increment primary keys
->To avoid duplicate IDs
->To make insertion easier

What is a Collection?

A collection is an object that can hold multiple items together as a single unit.

In simple words:
👉 Like a basket that can hold many items of the same or different types.

🔹 Why use Collections?

To store multiple values

To manipulate data easily (add, remove, search, sort)

More flexible than arrays

🔹 Types of Collections in C#
1️⃣ Non-Generic Collections (System.Collections)
Collection	Description
ArrayList	Dynamically sized array, can store any type of objects
Hashtable	Key-value pairs, fast lookup
Queue	FIFO (First In First Out)
Stack	LIFO (Last In First Out)

❌ No type safety (can store any type, may cause runtime errors)

2️⃣ Generic Collections (System.Collections.Generic)
Collection	Description
List<T>	Dynamic array, type-safe
Dictionary<TKey,TValue>	Key-value pairs, type-safe
Queue<T>	FIFO, type-safe
Stack<T>	LIFO, type-safe
HashSet<T>	Stores unique items only


Dictionary Key:
IT is used to store in a key-value pair 
key should be unique and value can be duplicated
used for
                faster retrival of data
                search with key will be easy
                No duplicate keys are allowed

List<t>:
                It is used to store multiple values at the same time
                no fixed in size
                easy to add ,remove and update items

HashSet:
                Hashset is a collection to store unique values
                no duplicates are allowed
                order is not gaurantee 
                very fast in performace


workflows->controller->interface->service->entityframework
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------
IActionResult:
                IActionResult represents the RESULT of an action method in an ASP.NET Core controller.
                It tells what response your API or MVC action will send back to the client

when we try to create or store data
   we will be using a concept called dto (DATA TRANSFFEER OBJECT) not directly injecting the module insted creating a dto and with help of that we will be creting a data
   why means->We use DTOs to protect, control, and shape data coming from the client.
   To avoid :
           1. Security risk (Over-posting)
           2. Database structure is exposed while we inject directly
           3.Validation becomes messy
...
Iemurable
  ->used in Interface
  ->it is used to list the data alone throught a inherited class


  explain about method hiding using "new" keyword:
  Method Hiding in C# happens when a child class defines a method with the same name as the parent class method, and instead of overriding it, it hides it using the new keyword.
  class Parent
{
    public void Show()
    {
...
In Method Hiding:
✔ Method called depends on Reference Type (compile-time)

In Method Overriding:
✔ Method called depends on Object Type (runtime)
